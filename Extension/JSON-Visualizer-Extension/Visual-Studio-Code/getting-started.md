...
## Visualize a JSON Data

After installing the extension, open the JSON document that you want to visualize:

1. Open the folder that contains your JSON file using **File ▸ Open Folder…**. Select a `.json` file from the Explorer to open it in the editor.
...
2. Locate the **Enable JSON Visualizer** icon (tooltip: *Enable JSON Visualizer*) in the editor toolbar at the top-right of the editor.

   ![Enable JSON Visualizer](Images/EnableJSON.png)

2. Click the icon to launch the visualizer. The extension reads the active JSON document and displays it as an interactive diagram in a web view panel alongside the JSON source.

3. Once the visualizer renders your JSON, use the following interactions to explore the structure:
